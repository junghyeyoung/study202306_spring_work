package com.acorn.spring05.users.service;
...
import com.acorn.spring05.users.dao.UsersDao;
import com.acorn.spring05.users.dto.UsersDto;
...
public class UsersServiceImpl implements UsersService{
   ...
   public void addUser(UsersDto dto) {
	...
	public void loginProcess(UsersDto dto, HttpSession session) {
		...
		if(resultDto != null) {
			//Bcrypt 클래스의 static 메소드를 이용해서 입력한 비밀번호와 암호화해서 저장된 비밀번호 일치여부를 알아내야한다.
		...
		if(isValid) { 
			...
			session.setAttribute("profile", resultDto.getProfile());
		}
	}
	...
	public void updateUserPwd(HttpSession session, UsersDto dto, ModelAndView mView) {
		...
		if(isValid) {
			...
			//암호화 된 비밀번호를 dto 에 다시 넣어준다.
			dto.setNewPwd(encodedNewPwd);
			//dto 에 로그인 된 아이디도 넣어준다.
			dto.setId(id);
